...
We welcome contributions! Please keep commits focused and well
described. Use the following format for commit messages:
...
names and defaults, and uses the same arithmetic ordering so a parity
test at `tolerance = 1e-8` passes.
...
A parity test in `tests/testthat/test-cpp-performance.R` is required. An
auto-routing `_fast` wrapper (see `R/fast-functions.R`) is encouraged
